...
// NewMysqlRepository ...
func NewMysqlRepository(conn *sql.DB, conng *gorm.DB) domain.PackageRepository {
	return &mysqlRepository{Conn: conn, Conng: conng}
}

func (m *mysqlRepository) Find(pkg *domain.Pkg) (pkgs []domain.Pkg, err error) {
	// err = m.Conng.Find(&pkgs, pkg).Error
	err = m.Conng.Model(pkg).Preload("SelectionProfile").Find(&pkgs, pkg).Error
	// err = m.Conng.Model(pkg).Preload("Radusergroup").Find(&pkgs).Error
	return
}

func (m *mysqlRepository) Save(pkg *domain.Pkg) (err error) {
	err = m.Conng.Save(&pkg).Error
	return
}

func (m *mysqlRepository) Create(ctx context.Context, pkg *domain.Pkg) (err error) {
	err = m.Conng.Create(&pkg).Error
	return
}

func (m *mysqlRepository) First(id uint) (pkg domain.Pkg, err error) {
	err = m.Conng.First(&pkg, "id = ?", id).Error
	return
}

func (m *mysqlRepository) Delete(pkgs []domain.Pkg) (err error) {
	err = m.Conng.Delete(&pkgs).Error
	return
}

...
func NewHandler(uc domain.SchedulerUsecase) {
	...
	go func() {
		for true {
			logrus.Infoln("start job for two minute")
			handler.TaskTwoMinutes()
			logrus.Infoln("time to sleep")
	...
	go func() {
		for true {
			logrus.Infoln("start job for one day")
			handler.TaskOneDay()
			logrus.Infoln("job one day done, time to sleep")
...
func (uc *SchedulerHandler) TaskTwoMinutes() {
	...
	uc.ucase.GuardMacBinding(ctx, wita)

	uc.ucase.GuardRadius(ctx, wita)
}
...
func (uc *SchedulerHandler) TaskOneDay() {
	...
	uc.ucase.IncreasePerform(ctx, wita)
	// ctx := context.Background()
